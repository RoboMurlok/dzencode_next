...
export default function CardProductShort({ items }: CardProps) {
  ...
  return (
    <div className="d-flex justify-content-between gap-1 px-3 py-2 bg-white border-bottom border-top">
      ...
      <div className="d-flex flex-column" style={{ width: 400 }}>
        <p className="fw-bolder text-black-50 m-0 small text-decoration-underline">
          {title}
        </p>
        <p className="fw-medium text-secondary m-0 small text-opacity-50">
          SN-{serialNumber}
        </p>
      </div>
      <div className={cssLeft} style={{ width: 80 }}>
        {isNew ? (
          <p className="fw-medium text-warning small m-0">свободен</p>
        ) : (
